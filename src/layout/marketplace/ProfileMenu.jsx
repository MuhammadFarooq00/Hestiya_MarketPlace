...
import { getBalance, getBalanceInPolygon, tokenAddress } from "../../abi";
...
const ProfileMenu = () => {
  ...
  const { userDetails, hasToken,setcheckUserLogedin,hasAddress,setHasAddress,setHasToken } = useContext(UserContext);
  const [checkClick, setCheckClick] = useState(false);
  const [checkBalance, setCheckBalance] = useState();
  const [balanceInPolygon, setBalanceInPolygon] = useState();
  ...
  useEffect(() => {
    if (userDetails) {
      getBalance(tokenAddress, hasAddress).then((balance) => {
        // console.log("check the balance in the wallet",balance);
        setCheckBalance(balance)
      });
       getBalanceInPolygon(tokenAddress, hasAddress).then((balance) => {
          // console.log("check the balance in polygon", balance);
          setBalanceInPolygon(balance);
        });
      // getBalanceInPolygon(tokenAddress, hasAddress).then((balance) => {
      //   console.log("check the balance in polygon", balance);
      //   setBalanceInPolygon(balance);
  ...
  }, [userDetails, hasAddress]);

  const reloadBalances = (e) => {
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    Promise.all([
      getBalance(tokenAddress, hasAddress),
      getBalanceInPolygon(tokenAddress, hasAddress)
    ]).then(([usd, pol]) => {
      setCheckBalance(usd);
      setBalanceInPolygon(pol);
  ...
  return (
    <>
      ...
      <Menu>
        ...
        <MenuList>
{/* {console.log("check the gender : ", userDetails?.gender?.toLowerCase)} */}
        

<div className="flex items-center justify-between gap-2 px-2 py-2 flex-nowrap">
  <Typography variant="small" className="font-medium text-nowrap">
    USD Balance :
  </Typography>
  <Typography variant="small" className="font-medium bg-blue-50 text-blue-gray-700 rounded-lg px-2 py-1">
    {loadingBalance ? (
      <span className="animate-pulse">Loading...</span>
    ) : (
      <span>
        {checkBalance && checkBalance.toString().slice(0, 5)}
      {" USD"}
      </span>
    )} 
  </Typography>
</div>
        <div className="flex items-center gap-2 px-2 pt-2 flex-nowrap">
  <Typography variant="small" className="font-medium text-nowrap">
    Polygon Balance :
  </Typography>
  <Typography variant="small" className="font-medium  bg-blue-50 text-blue-gray-700 rounded-lg px-2 py-1">
    {loadingBalance ? (
      ...
      <span>
        {balanceInPolygon && balanceInPolygon.toString().slice(0, 5)
} {" Pol"}
      </span>
    )} 
  </Typography>
...
<div
  role="button"
  tabIndex={0}
  onMouseDown={reloadBalances}
  className="flex bg-gray-200 mt-2 items-center gap-2 px-4 py-2 rounded-md cursor-pointer hover:bg-gray-300 transition-colors"
>
 <Refresh className=" transform "/>
