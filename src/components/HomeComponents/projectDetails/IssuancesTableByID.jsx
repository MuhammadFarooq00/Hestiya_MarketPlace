...
const IssuancesTableByID = ({ projectId }) => {
	...
	const fetchData = async (page = 1) => {
		...
		try {
			const res = await axios.get(
				`${apiUrl}activity-log/?page=${page}&project=${projectId}&ordering=${sortbackend}`
			);
			...
			// console.log("Fetched Data: abcd", res.data);
	...
	const handleProjectDetailPage = (
		...
		navigate(
			`/marketplace/hestiya-registory/issuance-detail?projectId=${id}&address=${address}&year=${year}&actionType=${actionType}&projectName=${projectName}&credits=${credits}&time=${time}&transhash=${transhash}`
		);
		// console.log("iddd", id);
	};
...
export default IssuancesTableByID;
